<?php
	...
	if($_POST['crl_insert']=="true"){
		echo " <a href='javascript:obj.showhide(\"div_edit\");obj.showhide(\"main_grid\");' onclick='obj.resetData(\"".implode('|',$flds_arr)."\",\"".implode('|',$users)."\",\"$_POST[idx]\");'><img src='".$root_path."images/insert1.gif' title='Insert'></a>";
	}
...
//	echo $_POST['crl_empty'];
	...
	<div id='div_edit' style='display:none'><br />";
	...
	while($flds_ls = mysql_fetch_assoc($fileds_result)){
		...
		if(count($fields_label) > 1){
			echo "<tr id='edit_$flds_ls[Field]'><td class='css_td header1'><label id='label_$flds_ls[Field]'>".ucfirst(trim($fields_label[$fl_idx]))."</label>";
			...
			echo "<tr id='edit_$flds_ls[Field]'><td class='css_td header1'><label id='label_$flds_ls[Field]'>".ucfirst($flds_ls[Field])."</label>";
			if(in_array($flds_ls[Field],$mandatory_fields) or $_POST['field_password'] == $flds_ls['Field']){
				echo " <span class='star'>*</span>";
			}
			echo "</td>";
		...
		if($size >= 100){
			...
			if($_POST['field_password'] == $flds_ls['Field']){
				echo "<td class='css_td'><input type='password' class='text_box' name='$flds_ls[Field]' id='$flds_ls[Field]'/></td></tr>";
				echo "<tr id='re_$flds_ls[Field]'><td class='css_td header1'><label id='label_re_$flds_ls[Field]'>Re-".ucfirst($flds_ls[Field])."</label>
	  ...
	  echo "</tr><tr><td colspan='2' align='center'><br /><input type='button' name='insert' class='btn_submit' id='insert' value=' Submit ' onclick='obj.saveRecord(\"$table_name\",\"$idx_filed_name\",\"$_POST[idx]\");'>&nbsp;&nbsp;<input type='button' name='insert' id='insert' class='btn_cancel' value=' Cancel ' onclick ='$(\"#div_edit\").slideToggle();
	$(\"#main_grid\").slideToggle();'></td></tr>
	</table>";
	
	$flds_list_str=implode('|',$flds_array);
	echo "</div>";
	echo "<div id='div_view' style='display:none'><br />";
	...
	while($flds_ls = mysql_fetch_assoc($fileds_result)){
		...
		echo "</tr><tr><td colspan='2' align='center'><br /><input type='button' name='insert' id='insert' class='btn_submit' value=' Update ' onclick ='obj.update(\"$table_name\",\"$idx_filed_name\",\"$_POST[idx]\")'>&nbsp;&nbsp;<input type='button' name='insert' id='insert' class='btn_cancel' value=' Back ' onclick ='$(\"#div_view\").slideToggle();
	$(\"#main_grid\").slideToggle();'></td></tr>
	</table>";
	
	echo "</div>";
	echo "</td></tr>";
	echo "<tr><td colspan='$cnt' align='center'><div id='main_grid' >";
	...
	foreach($arr as $arr1){
				...
				if(count($arr1)!= count(explode(',',$_POST['fields']))){
					if ($flds_array[$inc] == $idx_filed_name ){
						if($_POST['edit_del_flg']=="true")
							echo "<td class='css_td header'><input type='checkbox' id='del_all' name='del_all' value='$unique_ids[$i]' onclick='obj.setCheck(this.checked);'></td>";
					}
		...
		foreach($arr1 as $key=>$value){
			...
			if($key == $idx_filed_name){
				if($_POST['edit_del_flg']=="true")
					echo "<td class='css_td header'><input type='checkbox' name='del_multi[]' value='$unique_ids[$i]'></td>";
				
				echo "<th class='css_td header'><a href='javascript:void(0);' onclick='obj.onViewData(\"$idx_filed_name\",\"".$table_datas[$unique_ids[$i]]."\",\"$flds_list_str\",\"$_POST[idx]\")'>$value</a></th>";
			}
			else{
				echo "<td class='css_td' style='width:".(strlen($value)*6)."px;'>".ucfirst(trim($value))."</td>";
		...
		if($_POST['edit_del_flg']=="true"){
			...
			echo "<td class='css_td edit'><a href='javascript:void(0);' onclick='obj.onEdit(\"$table_name\",\"$idx_filed_name\",\"$unique_ids[$i]\",\"".$table_datas[$unique_ids[$i]]."\",\"$flds_list_str\",\"$_POST[idx]\")'><img class=img src='".$root_path."images/edit.gif' border=0 title='Edit'></a></td>";
