...
function PreviousDayDisabled (calendar, currentDate)
{
	...
	var evt_date=new Date(this.event_date.getUTCFullYear(),this.event_date.getUTCMonth()-1,this.event_date.getUTCDate()+1);
	str=currentDate.getUTCDate()==(this.event_date.getUTCDate()+1)&& this.event_date.getUTCMonth()==calendar.calendarDate.getUTCMonth()+1 && this.event_date.getUTCFullYear()==calendar.calendarDate.getUTCFullYear()?'$'+getAmount(cur_date,evt_date,calendar):'';
	...
	if(t!='day'){
		...
		var class1=sel_date?"cell_bg":"eventdate1";
		calHtml += ("<td id='td_"+currentDate.getUTCDate()+"' class=\""+class1+"\" title=' Event end date ' onclick=\"selectDate(" + calendar.name + ", " + currentDate.getUTCDate() + ","+(getAmount(cur_date,evt_date,calendar))+")\">");
		calHtml += ("<div class='div'>");
		calHtml += (currentDate.getUTCDate()+'</div><div class="rate1">$'+(getAmount(cur_date,evt_date,calendar)));
		calHtml += ("</div>");
		calHtml += ("</td>");
	}
	else{
...
function getAmount(cur_date,ev_date,calendar){
	...
	cur_days_diff= Math.ceil((ev_date.getTime()  - new Date().getTime())/(1000*60*60*24));
	days_diff=Math.ceil((ev_date.getTime())-(cur_date.getTime()))/(1000*60*60*24);
	
	init_amt=Math.ceil(cur_days_diff - days_diff)+calendar.price;
	return init_amt;
}
function RenderDayEnabled (calendar, currentDate, dayStyle)
{
	...
	if(getNumDays(cur_date,new Date())==0){
		this.idx++;
		class1=sel_date?"cell_bg":"eventdate1";
		calHtml += ("<td id='td_"+currentDate.getUTCDate()+"' class=\""+class1+"\" title=' TODAY ' onclick=\"selectDate(" + calendar.name + ", " + currentDate.getUTCDate() + ","+(getAmount(cur_date,evt_date,calendar))+")\">");
		calHtml += ("<div class='div'>");
		calHtml += (currentDate.getUTCDate()+'</div><span class="rate1">$'+(getAmount(cur_date,evt_date,calendar)));
		calHtml += ("</span><br><span class='sz'>TODAY</span>");
		calHtml += ("</td>");
	}
	else if(cur_date<evt_date && this.flag){
		this.idx++;
		class1=sel_date?"cell_bg":"div_enable";
		calHtml += ("<td id='td_"+currentDate.getUTCDate()+"' class=\""+class1+"\" onclick=\"selectDate(" + calendar.name + ", " + currentDate.getUTCDate() + ","+(getAmount(cur_date,evt_date,calendar))+")\">");
		calHtml += ("<div class='div'>");
		calHtml += (currentDate.getUTCDate()+'</div><div class="rate">$'+(getAmount(cur_date,evt_date,calendar)));
		calHtml += ("</div>");
		calHtml += ("</td>");
	}
	else{
...
function buildCalendarTable (calendar)
{
	...
	do
	{
		...
		for (i = 0;i < 7;i++)
		{
			...
			else if (currentDate.getUTCDate() < new Date().getUTCDate() && new Date().getUTCMonth()==calendar.calendarDate.getUTCMonth() &&  new Date().getUTCFullYear()==calendar.calendarDate.getUTCFullYear())
			{
...
function setSelectedField(id,date){
	...
	if(str_sel!=""){
		for(ii=0;ii<arr.length;ii++){
			var arr_1=arr[ii].split("$");
			var arr_2=arr_1[0].split("-");
			if(arr_2.length>1){
...
function getNumDays_p(cur_date,ev_date){
	days_diff=Math.floor((Math.ceil(cur_date.getTime())-Math.ceil(ev_date.getTime()))/(1000*60*60*24));
	return days_diff;
}