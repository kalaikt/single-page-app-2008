...
{
	t=currentDate_feature.getUTCDate()==(this.feature_eventdate.getUTCDate()+1)&& this.feature_eventdate.getUTCMonth()==calendarfeature.calendarfeatureDate.getUTCMonth()+1 && this.feature_eventdate.getUTCFullYear()==calendarfeature.calendarfeatureDate.getUTCFullYear()?"'eventdate' title='Event end date'":"day";
...
function RenderDayEnabled_feature (calendarfeature, currentDate_feature, dayStyle)
{
	...
	if(cur_date<evt_date && this.flag_Feature){
		...
		if(getNumDays(cur_date,new Date())==0){
			...
			calHtml += ("<td id='tdf_"+currentDate_feature.getUTCDate()+"' class=\"eventdate1\" title=' TODAY ' onclick=\"selectDate_Feature(" + calendarfeature.name + ", " + currentDate_feature.getUTCDate() + ","+currentDate_feature.getUTCDate()+")\">" );
			...
			var seldate=getSelDate(calendarfeature.attachedId,currentDate_feature);
			class_f=seldate?"cell_bg":"div_enable";
			calHtml += ("<td id='tdf_"+currentDate_feature.getUTCDate()+"' class=\""+class_f+"\" onclick=\"selectDate_Feature(" + calendarfeature.name + ", " + currentDate_feature.getUTCDate() + ","+currentDate_feature.getUTCDate()+")\">" );
			calHtml += ("<div id='td"+currentDate_feature.getUTCDate()+"'  >");
			calHtml += (currentDate_feature.getUTCDate()+'</div>');
			calHtml += ("</td>");
		}
...
function getSelDate(id,date){
	var str_sel_date=document.getElementById(id).value;
	var Field_f=0;
	if(str_sel_date!=""){
			arr_f=str_sel_date.split("-");
			if(eval(arr_f[0])==date.getUTCFullYear() && eval(arr_f[1]-1) == date.getUTCMonth() && eval(arr_f[2]) == date.getUTCDate()){
				Field_f=1;
			}
			else
				Field_f=0;
