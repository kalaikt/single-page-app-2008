...
class AjaxFileuploader {
	...
	public function showFileUploader($uploaderId) {
		if (in_array($uploaderId, $this->uploaderIdArray)) {
			...
			return '<form id="formName'.$uploaderId.'" name="formName'.$uploaderId.'" method="post" enctype="multipart/form-data" action="imageupload.php?dirname='.$this->uploadDirectory.'" target="iframe'.$uploaderId.'">
						<input type="hidden" name="id" value="'.$uploaderId.'" />							
						<span id="uploader'.$uploaderId.'" style="font-family:verdana;font-size:10;">
							<input class="text_box width" name="'.$uploaderId.'" size="60" type="file" value="'.$uploaderId.'" onchange=\'return people.uploadFile(this,"'.$this->uploadDirectory.'")\' />
							<input type="hidden" name="f_name" id="f_name" value="'.date('YmdHis').'">
							</span>
